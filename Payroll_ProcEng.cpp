...
int main() {
    ...
    while (getline(file, line)) {
        // Step 1: Display employee categorization (Code 1 functionality)
        if (line.find("The") == 0) {
        ...
        // Step 2: Calculate and display total rounded hours for F1, F2, F4 (Code 2 functionality)
        if (line.find("F1") != string::npos || line.find("F2") != string::npos || line.find("F4") != string::npos) {
            int totalHours = 0;
            int totalMinutes = 0;
